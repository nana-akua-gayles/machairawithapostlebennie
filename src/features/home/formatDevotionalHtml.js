...
const closeRe = /["'\u2018\u201c\u201d\u2019]/g;
 ...
 closeCandidates.push(closeMatch.index);
 }
 let found = null;
 let usedCloseIdx = -1;
 ...
 if (found) {
 const rawQuoteText = afterQuote.slice(0, usedCloseIdx).trim();
 const quoteText = rawQuoteText.replace(/["'\u2018\u2019\u201c\u201d]/g, '').trim();
...
function parseBibleReading(bibleReadingRaw) {
  const dualPlanMatch = bibleReadingRaw.match(
    /^\s*(\d{4})\s*1\s*YEAR\s*PLAN\s*([\s\S]*?)\s*2\s*YEARS?\s*PLAN\s*([\s\S]*)$/i
  );
  if (dualPlanMatch) {
    const [, , plan1Text, plan2Text] = dualPlanMatch;
    return {
      day: null,
      passages: null,
      readingPlans: [
        { label: '1 Year Plan', passage: plan1Text.trim() },
        { label: '2 Years Plan', passage: plan2Text.trim() },
      ],
    };
  }
  ...
  const passages = bibleReadingRaw
    ...
    .replace(/\n/g, ' ')
    .trim();
  return { day, passages, readingPlans: null };
}
...
function parseDeclarations(declareRaw) {
 ...
 const declarations = declarationLines.filter(Boolean).map((d) => stripShortcodes(d)).filter(Boolean);
 return { declarations, closing };
}
...
function buildFooterHtml(footerRaw) {
  ...
  const { day, passages, readingPlans } = parseBibleReading(bibleReadingRaw);
  const { declarations, closing } = parseDeclarations(declareRaw);
  const declareItems = declarations.map((d) => `<li>${d}</li>`).join('');
  const bibleReadingHtml = readingPlans
    ? readingPlans.map((p) =>
        `<p><span class="footerLabel">Bible Reading \u2014 ${p.label}</span> ${boldScriptureRefs(p.passage)}</p>`
      ).join('')
    : (passages
        ? `<p><span class="footerLabel">Bible Reading${day ? ` \u2014 Day ${day}` : ''}</span> ${boldScriptureRefs(passages)}</p>`
        : '');

  return (
    `<div class="footer">` +
    (digDeeper ? `<p><span class="footerLabel">Dig Deeper</span> ${boldScriptureRefs(digDeeper)}</p>` : '') +
    (wePray
      ? (splitDashPoints(wePray)
          ? `<p><span class="footerLabel">We Pray</span></p>` + splitDashPoints(wePray).map((p) => `<p>${boldScriptureRefs(p)}</p>`).join('')
          : `<p><span class="footerLabel">We Pray</span> ${boldScriptureRefs(wePray)}</p>`)
      : '') +
    bibleReadingHtml +
...
function processDevotionalHtml(rawHtml, { title, includeFooter = true } = {}) {
 ...
 let content = stripLeadingCss(rawHtml.trim());
 content = stripBarePlainTextUrls(content);
 ...
 content = stripNonEpisodeLinks(content2);
 ...
 ? paragraphize(boldScriptureRefs(convertDashLists(stripEmptyTags(verse.leading))))
 : null;
 const bodySource = hasKeyVerse ? verse.rest : content;
 const { body, footerHtml } = splitFooter(bodySource);
 const cleanedBody = boldScriptureRefs(convertDashLists(stripEmptyTags(body)));
...
module.exports = {
 ...
 findScriptureReference, stripPreamble, stripEmptyTags, matchCapsTitleRun, stripEpisodePrefix, convertDashLists,
 boldScriptureRefs, extractOpeningScripture, FOOTER_LABELS, splitFooter, paragraphize, fixMissingSentenceSpaces, fixGluedWords, fixWordGluedToQuote, fixDigitOrParenGluedToCapital, fixAllCapsGlue, stripShortcodes, breakSentencesForList, breakLongListItems, parseBibleReading, splitDashPoints,
 renderKeyVerseHtml, processDevotionalHtml, fixWordGluedToDigit, stripNonEpisodeLinks, isEpisodeNavLink, parseDeclarations,
stripLeadingCss,
};
