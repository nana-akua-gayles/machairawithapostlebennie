...
export const GroupDetailScreen = ({ route, navigation }) => {
  ...
  const {
    ...
    isLoading, loadError, hasMoreMessages, isLoadingMore, hasMoreMembers, isLoadingMoreMembers, requiresAuth,
  ...
  const mentionSuggestions = useMemo(() => {
    ...
    return members.filter((m) => m.user_id !== currentUser?.id && (m.profiles?.name || '').toLowerCase().startsWith(query)).slice(0, 5);
  }, [inputText, members, currentUser?.id]);

  const insertMention = (name) => {
    const newText = inputText.replace(/(?:^|\s)@(\w*)$/, (matched) => `${matched.startsWith(' ') ? ' ' : ''}@${name} `);
    ...
    <View style={styles.container}>
      ...
      <LinearGradient colors={[DEEP_PURPLE, '#1a1424']} style={styles.headerBackground}>
        <SafeAreaView edges={['top']} style={styles.safeHeaderContainer}>
          <View style={styles.headerTopRow}>
            <TouchableOpacity style={styles.headerIconButton} onPress={() => navigation.goBack()} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Go back">
            ...
            <View style={styles.headerTitleContainer}>
              ...
              <AppText style={styles.headerSubtitle} maxFontSizeMultiplier={1.3}>{membersCount} members • Learning Space</AppText>
            </View>

            <TouchableOpacity style={styles.headerIconButton} onPress={() => setShowDropdown(!showDropdown)} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="More options">
              <MoreVertical color="white" size={18} />
            </TouchableOpacity>
          </View>

          <View style={styles.tabBar}>
            <TouchableOpacity style={[styles.tabItem, activeTab === 'discussion' && styles.activeTabItem]} onPress={() => setActiveTab('discussion')} accessibilityRole="button" accessibilityLabel="Discussion tab" accessibilityState={{ selected: activeTab === 'discussion' }}>
              <MessageSquare color={activeTab === 'discussion' ? theme.primary : '#94A3B8'} size={15} />
              <AppText type="bold" style={[styles.tabText, activeTab === 'discussion' && styles.activeTabText]} maxFontSizeMultiplier={1.3}>Discussion</AppText>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.tabItem, activeTab === 'announcements' && styles.activeTabItem]} onPress={() => setActiveTab('announcements')} accessibilityRole="button" accessibilityLabel="Announcements tab" accessibilityState={{ selected: activeTab === 'announcements' }}>
              <Pin color={activeTab === 'announcements' ? theme.primary : '#94A3B8'} size={15} />
              <AppText type="bold" style={[styles.tabText, activeTab === 'announcements' && styles.activeTabText]} maxFontSizeMultiplier={1.3}>Announcements</AppText>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </LinearGradient>
      ...
      {!isOnline && (
        <View style={styles.offlineBanner}>
          <WifiOff color={theme.offlineText} size={13} />
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.mainLayer}>
        {requiresAuth ? (
          <View style={styles.loadingContainer}>
            <Lock color={theme.textSecondary} size={32} style={{ marginBottom: 12 }} />
            <AppText type="bold" style={styles.errorTitle}>Sign in required</AppText>
            <AppText style={styles.errorText}>Please sign in to view and join this discussion.</AppText>
            <TouchableOpacity style={styles.retryButton} onPress={() => navigation.goBack()} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Go back">
              <AppText type="bold" style={styles.retryButtonText}>Go Back</AppText>
            </TouchableOpacity>
          </View>
        ) : isLoading ? (
          <View style={styles.loadingContainer}>
          ...
          <View style={{ flex: 1 }}>
            {members.some(member => member.user_id === currentUser?.id) ? (
              <View style={{ flex: 1 }}>
                <FlatList
                  ...
                  renderItem={({ item, index }) => {
                    ...
                    const quotedAuthor = item.reply_preview?.authorName || (quoted?.user_id === currentUser?.id ? 'You' : quoted?.profiles?.name) || 'Member';
                            ...
                            <TouchableOpacity activeOpacity={0.9} onLongPress={handleLongPressMessage} style={[isMe ? styles.myMessageBubble : styles.peerMessageBubble, { maxWidth: undefined }]}>
                              ...
                              {isEditing ? (
                                <View style={{ width: '100%' }}>
                                  <TextInput style={styles.editMessageInput} defaultValue={item.content} onChangeText={(text) => setEditMessageText(text)} multiline maxLength={500} placeholderTextColor="#94A3B8" />
                                  <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 6 }}>
                                    ...
                                    <TouchableOpacity
                                      onPress={() => { const textToSave = editMessageText.trim() ? editMessageText : item.content; handleEditMessageManual(item.id, textToSave); }}
                                      disabled={isEditingMessage}
                                      style={{ backgroundColor: theme.primary, paddingVertical: 3, paddingHorizontal: 10, borderRadius: 4 }}
                                ...
                                <>
                                  {item.reply_to_id && (
                                    <View style={styles.quotedReplyBox}>
                                      ...
                                      <AppText numberOfLines={1} style={styles.quotedReplyText}>{quoted?.content || 'Original message'}</AppText>
                                    </View>
                                  )}
                                  ...
                                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                                    {item.status === 'sending' ? (
                                      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        <AppText style={[styles.messageTime, { marginLeft: 6 }]} maxFontSizeMultiplier={1.3}>Sending...</AppText>
                                      </View>
                                    ) : item.status === 'failed' ? (
                                      <TouchableOpacity onPress={() => retryFailedMessage(item.id)} onLongPress={() => discardFailedMessage(item.id)} style={{ flexDirection: 'row', alignItems: 'center' }} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} accessibilityRole="button" accessibilityLabel="Message failed to send. Tap to retry, long-press to discard.">
                                        <AlertCircle color="#D14343" size={12} />
                                        <AppText style={styles.failedText}>Failed · Tap to retry</AppText>
                                      </TouchableOpacity>
                                    ) : (
                                      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <AppText style={styles.messageTime} maxFontSizeMultiplier={1.3}>
                                          {new Date(item.created_at || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                          {item.edited_at ? ' · edited' : ''}
                                        </AppText>
                                        {isMe && item.status === 'sent' && (
                                          isReadByOthers(item) ? <CheckCheck color="#53BDEB" size={12} style={{ marginLeft: 4 }} /> : <Check color={theme.textSecondary} size={11} style={{ marginLeft: 4 }} />
                                        )}
                                      </View>
                                    )}
                ...
                {typingUsers.length > 0 && (
                  <View style={styles.typingIndicatorBar}>
                    <AppText style={styles.typingIndicatorText}>{typingUsers.length === 1 ? `${typingUsers[0].name || 'Someone'} is typing...` : `${typingUsers.length} people are typing...`}</AppText>
                  </View>
                )}
                ...
                {mentionSuggestions.length > 0 && (
                  <View style={styles.mentionSuggestionRow}>
                    {mentionSuggestions.map((m) => (
                      <TouchableOpacity key={m.user_id} onPress={() => insertMention(m.profiles?.name || 'Member')} style={styles.mentionSuggestionChip} accessibilityRole="button" accessibilityLabel={`Mention ${m.profiles?.name || 'Member'}`}>
                ...
                <View style={[styles.inputBarContainer, { paddingBottom: Math.max(insets.bottom, 10) }]}>
                  <View style={styles.inputWrapper}>
                    <TextInput style={styles.textInput} placeholder="Share a thought or message..." placeholderTextColor="#94A3B8" value={inputText} onChangeText={handleInputChange} multiline maxLength={500} accessibilityLabel="Message input" />
                    <TouchableOpacity style={[styles.sendButton, !inputText.trim() && { opacity: 0.5 }]} onPress={handleSendMessage} disabled={!inputText.trim() || isSending} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Send message">
              ...
              <View style={styles.nonMemberContainer}>
                <View style={styles.groupHeaderBanner}>
                  <View style={styles.bannerIconCircle}>
                    {groupIcon ? <Image source={{ uri: groupIcon }} style={{ width: 56, height: 56, borderRadius: 28 }} /> : <Users color={theme.primary} size={28} />}
                  </View>
                  <AppText type="bold" style={styles.bannerTitle}>{group?.name}</AppText>
                  <AppText style={styles.bannerDesc}>{group?.description || 'Welcome to our discussion space.'}</AppText>
                </View>
                ...
                <AppText style={styles.lockDesc}>Join this group to participate in the conversation and view messages.</AppText>
                <TouchableOpacity style={styles.joinDiscussionButton} onPress={handleJoinGroup} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Join group">
          ...
          <View style={{ flex: 1 }}>
            {members.some(member => member.user_id === currentUser?.id) ? (
                ...
                ListHeaderComponent={
                  isAdmin ? (
                    <View style={{ marginBottom: 24 }}>
                      <TouchableOpacity style={styles.modernPostButton} onPress={() => { setAnnouncementText(''); setShowAnnouncementModal(true); }} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="New announcement">
                  ...
                  return (
                    <View style={styles.rawAnnouncementItem}>
                      ...
                      <View style={{ flex: 1 }}>
                        {isEditing ? (
                          <View>
                            <TextInput style={styles.editAnnouncementInput} defaultValue={item.content} onChangeText={(text) => setEditText(text)} multiline maxLength={1000} placeholderTextColor="#94A3B8" />
                            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 8 }}>
                              ...
                              <TouchableOpacity
                                onPress={() => { const textToSave = editText.trim() ? editText : item.content; handleEditAnnouncementManual(item.id, textToSave); }}
                                disabled={isEditingAnnouncement}
                                style={{ backgroundColor: theme.primary, paddingVertical: 4, paddingHorizontal: 12, borderRadius: 6 }}
                                accessibilityRole="button"
                          ...
                          <>
                            ...
                            <View style={styles.rawMetaRow}>
                              ...
                              {isAdmin && (
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                  <TouchableOpacity onPress={() => handleStartEditing(item)} style={{ marginRight: 14, padding: 2 }} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} accessibilityRole="button" accessibilityLabel="Edit announcement">
              ...
              <View style={styles.nonMemberContainer}>
                ...
                <AppText style={styles.lockDesc}>Join this group to view announcements.</AppText>
                <TouchableOpacity style={styles.joinDiscussionButton} onPress={handleJoinGroup} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Join group">
                  <AppText type="bold" style={styles.joinDiscussionText}>Join Group</AppText>
                </TouchableOpacity>
              </View>
        ...
        {activeTab === 'members' && (
          <MembersTab members={members} membersCount={membersCount} group={group} isAdmin={isAdmin} refreshing={refreshing} onRefresh={onRefresh} onBack={() => setActiveTab('discussion')} onRemoveMember={handleRemoveMember} onLoadMore={loadMoreMembers} hasMore={hasMoreMembers} isLoadingMore={isLoadingMoreMembers} />
        )}

        {activeTab === 'admin' && isAdmin && (
          <AdminTab onBack={() => setActiveTab('discussion')} onManageParticipants={() => setActiveTab('members')} onChangeAvatar={handleChangeGroupAvatar} isUploadingAvatar={isUploadingAvatar} />
        )}
        </>
        )}
      ...
      {showDropdown && (
        <GroupDropdownMenu isAdmin={isAdmin} onClose={() => setShowDropdown(false)} onOpenAdmin={() => { setShowDropdown(false); setActiveTab('admin'); }} onShare={handleShareGroup} onDelete={handleDeleteGroup} onLeave={handleLeaveGroup} />
      )}
...
const getStyles = (theme) => StyleSheet.create({
  ...
  loadingText: { marginTop: 12, color: theme.textSecondary, fontSize: 13 },
  errorTitle: { fontSize: 16, color: theme.text, marginBottom: 6, textAlign: 'center' },
  errorText: { fontSize: 13, color: theme.textSecondary, textAlign: 'center', marginBottom: 16, lineHeight: 18 },
  retryButton: { backgroundColor: theme.primary, paddingVertical: 10, paddingHorizontal: 24, borderRadius: 8 },
  retryButtonText: { color: '#FFFFFF', fontSize: 13 },
  endOfHistoryText: { color: theme.textSecondary, fontSize: 11, textAlign: 'center', letterSpacing: 0.3 },
  offlineBanner: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: theme.offlineBg, paddingVertical: 6, paddingHorizontal: 12 },
  offlineBannerText: { color: theme.offlineText, fontSize: 11, marginLeft: 6, textAlign: 'center' },
  ...
  replyPreviewAuthor: { fontSize: 11, color: theme.primary },
  replyPreviewText: { fontSize: 12, color: theme.textSecondary },
  mentionSuggestionRow: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 16, paddingTop: 6, backgroundColor: theme.card },
  mentionSuggestionChip: { backgroundColor: theme.editInputBg, borderRadius: 14, paddingHorizontal: 10, paddingVertical: 5, marginRight: 6, marginBottom: 6 },
  mentionSuggestionText: { fontSize: 12, color: theme.text },
  inputBarContainer: { paddingHorizontal: 16, paddingTop: 10, paddingBottom: 10, backgroundColor: theme.card, borderTopWidth: 1, borderTopColor: theme.border },
  inputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: theme.background, borderRadius: 24, borderWidth: 1, borderColor: theme.border, paddingHorizontal: 16, paddingVertical: 4 },
  textInput: { flex: 1, maxHeight: 100, fontSize: 14, color: theme.text, paddingVertical: 6 },
  sendButton: { width: 34, height: 34, borderRadius: 17, backgroundColor: theme.primary, justifyContent: 'center', alignItems: 'center', marginLeft: 8 },
  announcementCard: { backgroundColor: theme.card, borderRadius: 24, padding: 22, marginBottom: 16, borderWidth: 1, borderColor: theme.border },
  ...
  modernPostButton: { height: 48, backgroundColor: theme.primary, borderRadius: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
  rawAnnouncementItem: { flexDirection: 'row', paddingVertical: 18, borderBottomWidth: 1, borderBottomColor: theme.border },
  rawIndicator: { width: 2, height: '100%', backgroundColor: theme.primary, borderRadius: 2, marginRight: 14 },
  rawAnnouncementText: { fontSize: 15, lineHeight: 22, letterSpacing: 0.2, color: theme.text },
  rawMetaRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 },
  rawDateText: { color: theme.textSecondary, fontSize: 10, letterSpacing: 1 },
  ...
  lockTitle: { fontSize: 18, color: theme.text, marginBottom: 6, textAlign: 'center' },
  lockDesc: { fontSize: 13, color: theme.textSecondary, textAlign: 'center', marginBottom: 20, lineHeight: 18 },
